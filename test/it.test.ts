...
describe('ItemRouter Integration Test', () => {
    ...
    it('POST /', async () => {
        ...
        const new_item = await request.get(`${url}/${res.body.id}`);
    ...
    it('PUT /:id', async () => {
        ...
        const response_exist = await request
            .put(`${url}/${id_exist}`)
            .set('Accept', 'application/json')
            .type('application/json')
            .send(data);
        ...
        const response_non_exist = await request
            .put(`${url}/${id_non_exist}`)
            .set('Accept', 'application/json')
            .type('application/json')
            .send(data);

        const new_item = await request.get(`${url}/${response_non_exist.body.id}`);
    ...
    it('DELETE /:id', async () => {
        ...
        await request.delete(`${url}/${id_exist}`).expect(204);
